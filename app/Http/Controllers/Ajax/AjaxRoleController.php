...
class AjaxRoleController extends Controller
{
    public function getRole(Request $request){
        if ($request->ajax()) {
            ...
            return DataTables::of($roles)
                ->addIndexColumn()
                ->addColumn('total_permission', function($row){
                    return count($row->permissions). " Total";
                })
                ->addColumn('action', function($row){ // Tambah kolom action untuk button edit dan delete
                    $btn = "<a class='btn btn-warning btn-sm d-inline me-1' href='".route('pengaturan.role.edit', $row->id)."' style='padding: 7px;padding-top: 5.5px; padding-left: 10px;padding-right: 10px' ><i class='bx bx-edit'></i></a>";
                    if ($row['name'] != 'Admin') {
                        $btn = $btn."<form action=".route('pengaturan.role.delete', $row->id)." method='POST' class='d-inline'>".csrf_field().method_field('DELETE')." <button type='submit' class='btn btn-danger btn-sm confirm-delete'><i class='bx bx-trash'></i></button></form>";
                    }
                    return $btn;
                })
                ->rawColumns(['action','total_permission'])
                ->make(true);
        }
    }
    ...
    public function getPermission(Request $request){
        if ($request->ajax()) {
            ...
            $permissions = Permission::select('permissions.id as permissions_id','name','menu.judul','menu.id')->join('menu','permissions.id_menu','=','menu.id')->orderBy('permissions.id', 'DESC')->get();

            // Return datatables
            return DataTables::of($permissions)
                ->addIndexColumn()
                ->addColumn('action', function($row){ // Tambah kolom action untuk button edit dan delete
                    $btn = "<a class='btn btn-warning btn-sm d-inline me-1' href='".route('pengaturan.permission.edit', $row->permissions_id)."' style='padding: 7px;padding-top: 5.6px; padding-left: 10px;padding-right: 10px' ><i class='bx bx-edit'></i></a>";
                    $btn = $btn."<form action=".route('pengaturan.permission.delete', $row->permissions_id)." method='POST' class='d-inline'>".csrf_field().method_field('DELETE')." <button type='submit' class='btn btn-danger btn-sm confirm-delete'><i class='bx bx-trash'></i></button></form>";
                    return $btn;
                })
                ->rawColumns(['action'])
                ->make(true);
