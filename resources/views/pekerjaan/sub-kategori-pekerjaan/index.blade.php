<x-layouts.app title="Pengaturan">
    ...
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Pengaturan / </span> Sub Kategori Pekerjaan</h4>
    ...
    <div class="row">
        ...
        <ul class="nav nav-pills flex-column flex-md-row mb-3">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="{{ route('sub-kategori-pekerjaan.index') }}"><i class="bx bxs-spreadsheet me-1"></i>Sub Kategori Pekerjaan</a>
            </li>
            <li class="nav-item">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <a href="{{ route('sub-kategori-pekerjaan.create') }}" class="mb-2 btn btn-primary">
                        <i class="bx bx-plus"></i> Tambah Sub Kategori Pekerjaan
                    </a>
                </div>
                    ...
                    <div class="row">
                        <div class="col-12 mb-3">
                            ...
                            <select class="form-select @error('id_kategori_pekerjaan') is-invalid @enderror" name="id_kategori_pekerjaan" id="id_kategori_pekerjaan">
                                ...
                                @foreach ($kategoriPekerjaan as $item)
                                    <option @if(old('id_kategori_pekerjaan')) selected @endif value="{{ $item->id }}">{{ $item->nama }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <div class="input-group input-group-merge">
                                <span class="input-group-text @error('name') redBorder @enderror"><i class="bx bxs-spreadsheet"></i></span>
                                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="Sub Kategori" value="{{ old('name')  }}" />
                            </div>
    ...
    <x-slot name="script">
        <script>
            ...
            $('#pekerja-table').DataTable({
                    ajax: {
                        method: "POST",
                        url: "{{ route('ajax.getSubKategoriPekerja') }}"
                    },
                    processing: true,
                    serverSide: true,
                    columns: [
        ...
        <script>
            ...
            function editData(e){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('ajax.getEditSubKategoriPekerja') }}",
                    data: {
                    ...
                    success: function (response) { // Jika ajax sukses dan memberikan respon
                        ...
                        var url = "{{ route('sub-kategori-pekerjaan.update', ':id') }}"; // Action pada form edit
                        url = url.replace(':id', subKategori.id );
                        $("#formEdit")[0].reset();
                        $('#formEdit').attr('action', url);
                        $('#modalPekerjaEdit').modal('show');
                        $('#name').val(subKategori.nama);
                        $('#id_kategori_pekerjaan').val(subKategori.id_kategori_pekerjaan).trigger('change');
                    }
                });
            }
